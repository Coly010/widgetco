...
export class Company {
  ...
  readonly maxTicketValue: number = 0.18;
  readonly minTicketValue: number = 0.07;
  ...
  private newBuildingCostFluctuation: number = 0.2;
  ...
  closeTickets(numTickets: number): void {
    ...
    this.capital +=
      numTickets *
      (Math.random() *
        (this.maxTicketValue - this.minTicketValue + this.minTicketValue));
  }
  ...
  getNewHireCost(): number {
    return (
      Math.random() * (this.maxNewHireCost - this.minNewHireCost) +
      this.minNewHireCost
    );
  }
  ...
  canFormSmallTeam(): boolean {
    return (
      this.developers.length >= this.smallTeamSize &&
      this.smallTeams.length < this.maxSmallTeams
    );
  }

  canFormMediumTeam(): boolean {
    return (
      this.smallTeams.length >= this.necessarySmallTeamsToFormMediumTeam &&
      this.mediumTeams.length < this.maxMediumTeams
    );
  }
  ...
  canFormLargeTeam(): boolean {
    ...
    let devsPerLargeTeam =
      this.smallTeamSize *
      this.necessarySmallTeamsToFormMediumTeam *
      this.necessaryMediumTeamsToFormLargeTeam;
    let largeTeamLimit =
      (buildingLimit / devsPerLargeTeam) * (this.buildingCount + 1);

    return (
      this.mediumTeams.length >= this.necessaryMediumTeamsToFormLargeTeam &&
  ...
  canHireNewDeveloper(): boolean {
    return (
      ...
      this.developers.length < this.maxIndividualContributors
    );
  }
  ...
  canHireNewHiringManager(): boolean {
    ...
    if (
      this.largeTeams.length >= requirement &&
      this.hiringMgrs.length < this.maxHiringManagers
    ) {
      return true;
    }
  ...
  canFormHRTeam(): boolean {
    let hrTeamPerBuildingLimit = Math.max(
      this.necessaryHrTeamsForNewBuilding,
      (this.buildingCount + 1) * this.necessaryHrTeamsForNewBuilding
    );
    return (
      this.hiringMgrs.length >= this.necessaryHiringManagersForHRTeam &&
  ...
  calculateNewBuildingCost(): number {
    let min =
      this.newBuildingCost -
      this.newBuildingCost * this.newBuildingCostFluctuation;
